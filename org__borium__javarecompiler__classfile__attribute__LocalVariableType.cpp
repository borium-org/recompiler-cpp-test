...
namespace org::borium::javarecompiler::classfile::attribute
{
	...
	Pointer<Class> LocalVariableType::__thisClassStatic;

	void LocalVariableType::__ClassInit()
	{
		...
		__thisClassStatic = new Class(
			"org::borium::javarecompiler::classfile::attribute::LocalVariableType",
			java::lang::Object::__thisClassStatic);

		java::lang::Object::__ClassInit();
	}

	LocalVariableType::LocalVariableType(Pointer<ByteInputStream> in, Pointer<ConstantPool> cp) :
		Object() //
		, startPc(0) //
		, length(0) //
		, nameIndex(0) //
	...
	void LocalVariableType::dump(Pointer<IndentedOutputStream> stream)
	{
		...
		stream->println(temp_000C->append(createString(": "))->append(this->signature)->toString());
		stream->indent(1);
